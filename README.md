...
7. The test to determine whether there are violations of rule 2 desctibed above is done by DenseLevel objects:
...
8. Lateral connectivity: For each DesnseLayer:
...
9. Materialize Dense layers. **(This is a top down operation starting with InputLevel and proceeding to the last hidden layer in the network)**
10. Create output layer.
11.Compile model.
12. Fit the model, save the oracles, and save the model as a daved Keras model.
13. Iterate through the results and find best the model ad metrics.  
