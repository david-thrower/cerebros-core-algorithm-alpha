...
parameter nuomenclature:
1. level_number of object the param pertains to: int, 'NA'
   if global not applicable to a level or specific subunit thereof.
2. "-"
3. Name of Level object's class (e.g. DenseLevel) or 'NA' if global param not
   related to a specific Level
4. "-"
4. Unit object index position on the Level it belongs(unit_number)
5 "-"
5. Name of Unit object's class: g.g. DenseUnit
6. "-"
6. Common name of parameter(e.g. learning_rate or predecessor_connetion)
7. "-"
8. Upstream level_numer this param sets connections to members of, or 'NA'
   if not applicable
8. "-"
9. Replicte for multiple seletions. (Talos and keras_tuner do not
   support multiple selections. If there is parameter expecting 5 selections
   with replacement, then we will need to make 5
   replicates: param-selection1, parm-slection2 ...,
   one for each expected selection then concatenate them)
