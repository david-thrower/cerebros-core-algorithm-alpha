...
Traceback (most recent call last):
  File "/home/runner/work/cerebros-core-algorithm-alpha/cerebros-core-algorithm-alpha/phishing_email_detection_gpt2.py", line 293, in <module>
    position_embedding = InterleavedRoPE(
                         ^^^^^^^^^^^^^^^^
  File "/opt/hostedtoolcache/Python/3.11.11/x64/lib/python3.11/site-packages/keras/src/utils/traceback_utils.py", line 70, in error_handler
    raise e.with_traceback(filtered_tb) from None
  File "/tmp/__autograph_generated_file1glgbmnr.py", line 13, in tf__call
    x = ag__.converted_call(ag__.ld(apply_rotary_pos_emb), (ag__.ld(x), ag__.ld(sin), ag__.ld(cos)), None, fscope)
        ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  File "/tmp/__autograph_generated_fileqgikeir3.py", line 10, in tf__apply_rotary_pos_emb
    x_rotated = ag__.ld(x) * ag__.ld(cos) + ag__.converted_call(ag__.ld(rotate_half), (ag__.ld(x),), None, fscope) * ag__.ld(sin)
                ~~~~~~~~~~~^~~~~~~~~~~~~~
ValueError: Exception encountered when calling layer "interleaved_ro_pe" (type InterleavedRoPE).
in user code:
    File "/home/runner/work/cerebros-core-algorithm-alpha/cerebros-core-algorithm-alpha/phishing_email_detection_gpt2.py", line 264, in call  *
        x = apply_rotary_pos_emb(x, sin, cos)
    File "/home/runner/work/cerebros-core-algorithm-alpha/cerebros-core-algorithm-alpha/phishing_email_detection_gpt2.py", line 249, in apply_rotary_pos_emb  *
        x_rotated = x * cos + rotate_half(x) * sin
    ValueError: Dimensions must be equal, but are 24 and 12 for '{{node interleaved_ro_pe/mul}} = Mul[T=DT_FLOAT](Placeholder, interleaved_ro_pe/rotary_embedding/strided_slice_2)' with input shapes: [?,1024,24], [1024,12].
Call arguments received by layer "interleaved_ro_pe" (type InterleavedRoPE):
  • x=tf.Tensor(shape=(None, 1024, 24), dtype=float32)
```
