...
### Each 20 or so text samples takes about 25 min to run a nas study on 
### 8 CPU / 32 GB RAM at the current scale. 
### Do the math as to how many samples you want to run

MAX_TRAIN_PLUS_TEST_SAMPLES = 150
...
# Guard rail for sanity in small scae testing:
if MAX_TRAIN_PLUS_TEST_SAMPLES:
    non_instruct_samples = non_instruct_samples[:MAX_TRAIN_PLUS_TEST_SAMPLES]
...
##### RESERVED FOR FUTURE USE (Instruct training proof of concept)
...
### Cerebros Text Embedding Base Model custom layers ######  
...
# helper functions for iRoPE (interleaved Rotary Positional Embedding)
...
# interleaved Rotary Postional Embedding layer (iRoPE)
@tf.keras.utils.register_keras_serializable()
