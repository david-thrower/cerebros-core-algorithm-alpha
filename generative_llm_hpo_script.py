...
# from old.random_search import trial
...
DATE_0 = "2025-12-21"
EXPERIMENT_NAME = "stacked-transformer"
SAMPLE_SIZE_i_a = 10
SAMPLE_SIZE_i_b = 20
DATA_SET_NAME = "WEB-Bible"
ADDITIONAL_METADATA = "2-levels"
EXPERIMENT_ITERATION = "0001"
EXPERIMENT_NAME = f"{DATE_0}-{EXPERIMENT_NAME}--{ADDITIONAL_METADATA}--{DATA_SET_NAME}-ia-{SAMPLE_SIZE_i_a}-ib-{SAMPLE_SIZE_i_b}--{EXPERIMENT_ITERATION}-a"
...
PHASE_I_A_SAMPLES_TO_CREATE = SAMPLE_SIZE_i_a

# Samples to use for the main training stage
PHASE_I_B_SAMPLES_TO_CREATE = SAMPLE_SIZE_i_b
...
def objective(trial):
    ...
    from cerebrosllmutils.llm_utils import (prepare_data,
                                            ...
                                            StackableChunkingAttentionBlock)
    ...
    num_lateral_connection_tries_per_unit = trial.suggest_int('num_lateral_connection_tries_per_unit', 5, 35)
    ...
    epochs = trial.suggest_int('epochs', 5, 75)
    ...
    maximum_units_per_level = trial.suggest_int('maximum_units_per_level', minimum_units_per_level, 7)

    # Number of units in each Dense layer:
    minimum_neurons_per_unit = 2
    maximum_neurons_per_unit =  trial.suggest_int('maximum_neurons_per_unit', minimum_neurons_per_unit, 7)
    ...
    INITIAL_LR_STAGE_I_B =\
            trial.suggest_float('INITIAL_LR_STAGE_I_B', 0.0001, 0.05, log=True)

    # A fixed number for the initial warmup

    WARMUP_STEPS = trial.suggest_int("WARMUP_STEPS", 500, 2000)  # Generally between 500 and 2000
    FIRST_DECAY_STEPS_STAGE_I_B = trial.suggest_int("FIRST_DECAY_STEPS_STAGE_I_B", WARMUP_STEPS, 3000)
    ...
    EMBEDDING_N = trial.suggest_int('embedding_n',5, 10)
    ...
    STANDARD_ATTENTION_DROPOUT_RATE = trial.suggest_float("STANDARD_ATTENTION_DROPOUT_RATE",
                                                          0.0,
                                                          0.95)

    # There will be NUM_STACKABLE_ATTENTION_LAYERS + 1
    # Attention layers.
    NUM_STACKABLE_ATTENTION_LAYERS = trial.suggest_int("NUM_STACKABLE_ATTENTION_LAYERS", 1, 4)
    ...
    K_PROJ = trial.suggest_categorical("K_PROJ", [2, 4, 5, 8, 10])

    # An intermediate projection between the attention block and the final base model projection
    GNN_OUTPUT_FEATURES_PER_TOKEN = trial.suggest_int("GNN_OUTPUT_FEATURES_PER_TOKEN",1,3)
    ...
    for i in range(NUM_STACKABLE_ATTENTION_LAYERS):
        embedded = StackableChunkingAttentionBlock(
    ...
    standard_attention_final = SingleHeadChunkedAttentionScalarOutput(d_model=EMBEDDING_DIM,
                                                                k_proj=K_PROJ,
                                                                name="standard_attention_head_final")(embedded)
    ...
    for i in range(NUM_STACKABLE_ATTENTION_LAYERS):
        position_embedding = StackableChunkingAttentionBlock(
            d_model=EMBEDDING_DIM,
            k_proj=K_PROJ,
            dff=D_FF,
            dropout_rate=STANDARD_ATTENTION_DROPOUT_RATE,
    ...
    irope_attention_final = SingleHeadChunkedAttentionScalarOutput(d_model=EMBEDDING_DIM,
                                                             k_proj=K_PROJ,
                                                             name="irope_attention_head_final")(position_embedding)

    # --- CORRECTED Gating Fusion Strategy using a Keras Layer ---
    fusion_layer = MeanAttentionFusion(name="mean_attention_fusion")
    combined_attention = fusion_layer([standard_attention_final, irope_attention_final])
